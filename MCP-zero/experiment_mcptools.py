...
def test_llm_retrieval(
    ...
    # 读取系统提示词
    system_prompt_path = os.path.join(output_dir, "prompt_guide", "system_vanilla.prompt")
    system_prompt = read_text_file(system_prompt_path)

    # 读取用户提示词模板
    user_prompt_template_path = os.path.join(output_dir, "prompt_guide", "system_ours_mcptools.prompt")
    ...
    if extracted_server_desc and extracted_tool_desc:
        ...
        try:
            ...
            if matched_tools:
                ...
                matched_server = next(
                    (s for s in matched_servers if s["server"]["name"] == matched_tool["server_name"]), None)
...
def run_grid_search(
    ...
    for (position_index, sample_size) in grid_points_list:
        ...
        result, _ = test_llm_retrieval(
            ...
            model_name="gpt-4-turbo"  # 修改为使用OpenAI模型
        )
